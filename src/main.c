...
//#define EEP_END (EEP_MEM - 1) //адрес последней ячейки епром
//#define N_MAX 50 //макс. количество устройств в епром, зависит от N_NAME, т к. sizeof (device) = (12 + N_NAME), а в епром 1024. Т.о. N_MAX < (1024-2)/(12 + N_NAME)
    ...
    while(1)
    {
        ...
        for (i = 0; i< n; i++) //начинаем опрос с первого датчика (от 0)
        {
            switch (press_time) // блок детектирования нажатия кнопки и переходов по длительности жима
            {
                case QUICK :
                    lcd_clr(); // очистка дисплея
                    send_arr_to_LCD_XY(utoa_fast_div(n_max, digits), 0, 0);
